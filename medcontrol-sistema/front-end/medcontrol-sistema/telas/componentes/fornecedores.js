...
// Script relacionado aos valores dos dashboard de fornecedores:------------------------------------------------------
  setTimeout(() => {
      ...
      const endpoints = {
          total: [
              "https://medcontrol-backend.onrender.com/dashboard_fornecedores_total",
              "http://localhost:3001/dashboard_fornecedores_total"
          ],
          ativos: [
              "https://medcontrol-backend.onrender.com/dashboard_fornecedores_ativos",
              "http://localhost:3001/dashboard_fornecedores_ativos"
          ],
          inativos: [
              "https://medcontrol-backend.onrender.com/dashboard_fornecedores_inativos",
              "http://localhost:3001/dashboard_fornecedores_inativos"
          ]
      };

      // Função genérica para buscar e preencher
      async function buscarDados(endpointsLista, elemento, campo) {
          ...
          elemento.textContent = "Erro ao buscar fornecedores";
          return false;
      }

      // ✅ Executa cada busca separadamente
      buscarDados(endpoints.total, fornecedoresTotal, "total_fornecedores");
      buscarDados(endpoints.ativos, fornecedoresAtivos, "total_fornecedores_ativos");
      buscarDados(endpoints.inativos, fornecedoresInativos, "total_fornecedores_inativos");
  ...
  setTimeout(() => {
    const form = document.querySelector(".cadastro-fornecedor-modal");
    const table = document.querySelector(".fornecedores-tabela table tbody");
    const modal = document.querySelector("#NovoFornecedor");
    ...
    form.addEventListener("submit", async (event) => {
      ...
      if (!/^\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}$/.test(cnpj)) {
        alert("Por favor, insira um CNPJ válido. Modelo: XX.XXX.XXX/XXXX-XX");
        return;
      }
      ...
      // Lista de endpoints (online primeiro, depois local)
      const endpoints = [
        "https://medcontrol-backend.onrender.com/cadastro_fornecedores", // online
        "http://localhost:3001/cadastro_fornecedores"                    // local
      ];
      ...
      for (const url of endpoints) {
        try {
          ...
          console.error(`❌ Erro ao cadastrar fornecedor em ${url}:`, error.message);
        }
      }

      if (!sucesso) {
        alert("❌ Erro ao cadastrar fornecedor (nenhum servidor respondeu).");
      }
    });
  }, 0);
  ...
  setTimeout(() => {
    ...
    const endpoints = [
      "https://medcontrol-backend.onrender.com/tabela_fornecedores", // online
      "http://localhost:3001/tabela_fornecedores"                    // local
    ];
    ...
    if (!sucesso) {
      console.error("❌ Nenhum servidor respondeu para tabela_fornecedores.");
    }
  }, 0);
  ...
  setTimeout(() => {
      ...
      tbody.addEventListener("click", async (event) => {
          ...
          form.addEventListener("submit", async (event) => {
              ...
              // Lista de endpoints (online primeiro, depois local)
              const endpoints = [
                  `https://medcontrol-backend.onrender.com/editar_fornecedores/${id_fornecedor}`, // online
                  `http://localhost:3001/editar_fornecedores/${id_fornecedor}`                    // local
              ];
              ...
              for (const url of endpoints) {
                  try {
                      ...
                      // Corrigindo índices das colunas
                      row.querySelector("td:nth-child(1)").textContent = data.nome_fornecedor;
                      row.querySelector("td:nth-child(2)").textContent = data.cnpj;
                      row.querySelector("td:nth-child(3)").textContent = data.contato;
                      ...
                      console.error(`❌ Erro ao editar fornecedor em ${url}:`, error.message);
                  }
              }

              if (!sucesso) {
                  alert("❌ Erro ao editar fornecedor (nenhum servidor respondeu).");
              }
  ...
  setTimeout(() => {
    ...
    table.addEventListener('click', async (event) => {
      if (event.target.classList.contains('btn-fornecedores-excluir')) {
        ...
        // Lista de possíveis endpoints (online primeiro, depois local)
        const endpoints = [
          `https://medcontrol-backend.onrender.com/deletar_fornecedor/${id_fornecedor}`, // online
          `http://localhost:3001/deletar_fornecedor/${id_fornecedor}`                    // local
        ];
        ...
        for (const url of endpoints) {
          try {
            ...
            console.error(`❌ Erro ao excluir fornecedor em ${url}:`, error.message);
          }
        }

        if (!sucesso) {
          alert("❌ Erro ao excluir fornecedor (nenhum servidor respondeu).");
        }
      }
    });
  }, 0);
